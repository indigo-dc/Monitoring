...
public class IamClient {
	...
	private static OIDCTokens getAccessToken(String location, AuthorizationGrant grant, String clientId,
			String clientSecret) {
		try {
	 ...
	 */
	public static OIDCTokens getAccessToken(String location, String username, String password, String clientId,
			String clientSecret) {
		AuthorizationGrant grant;
		String grantType;
		try {
			grantType = PropertiesManager.getProperty(ProbesTags.IAM_GRANT_TYPE);
			if (grantType.equals(ProbesTags.GRANT_TYPE_CLIENT_CREDENTIALS))
				grant = new ClientCredentialsGrant();
			else
				grant = new ResourceOwnerPasswordCredentialsGrant(username, new Secret(password));
		} catch (NullPointerException e) {
			grant = new ResourceOwnerPasswordCredentialsGrant(username, new Secret(password));
		}
		
		return getAccessToken(location, grant, clientId, clientSecret);
	}

	/**
	 * Utility method that will return an access token from parameters provided in
	 * the configuration file. - iam.location: Location of the IAM server. -
	 * iam.username: Username for the authorized user. - iam.password: Password for
	 * the authorized user. - iam.clientid: Client ID to authorize. -
	 ...
	 */
	public static OIDCTokens refreshToken(String iamLocation, String refreshTokenValue, String clientId,
			String clientSecretValue) {

		return getAccessToken(iamLocation, new RefreshTokenGrant(new RefreshToken(refreshTokenValue)), clientId,
				clientSecretValue);
	}

	/**
	 * Gets new access and refresh tokens by getting the IAM information from the
	 * properties file.
	 *
	...
	public static OIDCTokens refreshToken(String refreshToken) {
		return refreshToken(PropertiesManager.getProperty(ProbesTags.IAM_LOCATION), refreshToken,
				PropertiesManager.getProperty(ProbesTags.IAM_CLIENTID),
				PropertiesManager.getProperty(ProbesTags.IAM_CLIENTSECRET));
	}
}
