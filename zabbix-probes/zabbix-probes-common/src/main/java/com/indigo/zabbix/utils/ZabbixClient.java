...
public class ZabbixClient {
	 ...
	 *            mock wrapper sender.
	 */
	public ZabbixClient(String category, String group, String template, ZabbixWrapperClient wrapperClient,
			ZabbixSender sender) {
	...
	/**
	 * Default constructor that will read the information from the configuration
	 * properties.
	 */
	public ZabbixClient(String category, String group, String template) {
		this(PropertiesManager.getProperty(ProbesTags.ZABBIX_WRAPPER_ENDPOINT), category, group, template,
				PropertiesManager.getProperty(ProbesTags.ZABBIX_HOST),
				new Integer(PropertiesManager.getProperty(ProbesTags.ZABBIX_PORT, ZABBIX_DEFAULT_PORT.toString())));
	}

	/**
	 * Default constructor.
	 */
	public ZabbixClient(String wrapperEndpoint, String category, String group, String template, String zabbixHost,
			Integer zabbixPort) {
				...
				if (zabbixHost != null) {
					...
					try {
						...
						if (!sendResult.success()) {
							/*
							 * Sometimes zabbix fails to process data the first
							 * time it's sent. Specially when the host is first
							 * registered. The reason why? It's a mystery and
							 * given the huge amount of data we have to debug
							 * the situation, let's just try to send it once
