...
public class ZabbixClient {
  ...
  public ZabbixClient(String category, String group, String template) {
    this(PropertiesManager.getProperty(ProbesTags.ZABBIX_WRAPPER_ENDPOINT), category, group,
        template, PropertiesManager.getProperty(ProbesTags.ZABBIX_HOST),
        new Integer(PropertiesManager.getProperty(
            ProbesTags.ZABBIX_PORT, ZABBIX_DEFAULT_PORT.toString())));
  }
        ...
        if (zabbixHost != null) {
          ...
          try {
            ...
            if (!sendResult.success()) {
               ...
               * given the huge amount of feedback we have to debug the situation, let's just try to
               * send it once again before failing.
               */
