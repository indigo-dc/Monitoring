...
public class OpenStackClient {
  ...
  public OpenStackClient(String keystoneLocation, String providerName) {
    ...
    String certificatesTrustStorePath = PropertiesManager
                                            .getProperty(OpenstackProbeTags.JAVA_KEYSTORE);
  ...
  protected void checkCredentials(String providerName) {
    ...
    for (CloudProvidersZone zone : osconfig.getMonitoringZones().getCloudProvidersZones()) {
      if (zone.getName().equalsIgnoreCase(providerName)
              && (zone.getPassword() != null || zone.getTenant() != null
  ...
  public CreateVmResult createVm() {
    ...
    try {
      if (Boolean.parseBoolean(PropertiesManager
                                   .getProperty(OpenstackProbeTags.WAIT_FOR_CREATION))) {
  ...
  private void pollerDelete(/* Server server, */ String instanceName)
      throws TimeoutException, InterruptedException {
  ...
  protected DeleteVmResult deleteVm(String vmId) throws TimeoutException, InterruptedException {
    ...
    // Feed monitoring info
    return new DeleteVmResult(resultMap.get(AVAILABILITY_STATUS),
                                 resultMap.get(HTTP_RESPONSE_CODE), responseTime);
  }
  ...
  private void tryToDeleteInstance(CreateVmResult createVmInfo) {
    ...
    try {
      ...
      log.debug("Timeout or interrupted operation when deleting the instance: "
                    + e.getMessage());
    }
  }
