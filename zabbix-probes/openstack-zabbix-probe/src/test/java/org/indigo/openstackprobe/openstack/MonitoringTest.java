...
public class MonitoringTest {
  ...
  public void prepareMockOpenstackServer() throws IOException {
    System.out.println(
        "Setting up testing environment and prepare the prepareMockOpenstackServer...");

    //		system = Mockito.mock(System.class);
    System.setProperty("jsse.enableSNIExtension", "false");
    System.setProperty(
        "javax.net.ssl.trustStore", "C:/Program Files/Java/jdk1.8.0_121/jre/lib/security/cacerts");
    ...
    //				"Openstack-Location:
    // http://cloud.recas.ba.infn.it:8774/compute/78d9ecb64353402bb621b569891c633a");
    ...
    Mockito.when(keystoneMock.credentials(Mockito.anyString(), Mockito.anyString()))
        .thenReturn(keystoneMock);
    ...
    Mockito.when(invocationBuilder.get()).thenReturn(responseGet);
    Mockito.when(responseGet.getStatus()).thenReturn(200);
    String listImagesResponse =
        "{\"total_rows\":72,\"offset\":6,\"rows\":[{\"id\":\"7efc59c5db69ea67c5100de0f726d41e\",\"key\":"
    ...
    Mockito.when(serverMocked.getName())
        .thenReturn(
            "vMOpenstackZabbixProbe_" + new BigInteger(37, new SecureRandom()).toString(16));
  ...
  public void monitoringOperationsShouldReturnCompleteResult() {
    ...
    //		Assert.assertNotNull("The inspectVM result should exist.",
    // result.getInspectVmElement().getInspectVmResult());
    //		Assert.assertNotNull("The inspectVM result should exist.",
    ...
    //		Assert.assertNotNull("The deleteVM result should exist.",
    // result.getDeleteVmElement().getDeleteVmResult());
    //		Assert.assertNotNull(result.getOsInstanceList());
    //		Assert.assertEquals("The general availability result should be 1", 1,
    // result.getGlobalAvailability());
  }

  @Test
  public void testFlavorSmall() {
    OpenStackClient openstackClient =
        Mockito.spy(new OpenStackClient(mockClient, keystoneMock, flavorMockId, imageMockId));
  ...
  public void testFlavorUnempty() {
    OpenStackClient openstackClient =
        Mockito.spy(
            new OpenStackClient(mockClientFailure, keystoneMock, flavorMockId, imageMockId));
