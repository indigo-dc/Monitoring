...
/**
 * This class aims at implementing the management of the threads to be used for the monitoring
 * operations. It follows a singleton pattern, so only one instance of the class will exist.
 *
 * @author ATOS
 */
public class ProbeThread {
  ...
  /**
   * It provides the current ProbeThread instance and, in case it does not exist, it creates a new
   * instance and gives it as the result.
   *
  ...
  /**
   * It provides the current ProbeThread instance and, in case it does not exist, it creates a new
   * instance and gives it as the result. This method is for unit testing purposes.
   *
   * @return The current single instance of the ProbeThread
   */
  ...
  /**
   * It manages the process of retrieving Cloud providers' information, preparing the treads pool
   * and launching monitoring operations per each provider.
   */
  public void startMonitoringProcess(String[] args) {

    try {
      PropertiesManager.loadProperties("occiprobe.properties", args);
      ...
      if (cmdb != null) {
                ...
                .forEach(
                    entry -> {
                      if (entry.getValue().success()) {
                        System.out.println("Successfully sent metrics for host " + entry.getKey());
                      } else {
                        System.out.println("Error sending metrics for host " + entry.getKey());
                      }
                    });
        ...
        if (provider != null
          ...
          System.out.println(
              "OCCI probe configuration incomplete. "
  ...
  public static void main(String[] args) {
    new ProbeThread().startMonitoringProcess(args);
  }
}
